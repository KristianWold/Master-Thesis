...
%================================================================
\subsection{DNNs}\label{sec:DNNs}
%================================================================
...
%================================================================
Because of the flexibility of the framework and compatability of the different layers, it is possible to make hybrid models by combining \textbf{Dense} and \textbf{QLayer} layers using
...
network = Neuralnetwork(layers)
\end{lstlisting}
I this code, the initial dense layer is used to reduce 64 feature to 4, a form a learned feature reduction. This may be a beneficial approach which we will study later, as feeding 64 features directly into a QCN would require 64 qubits using qubit encoding. Note that the use of tanh activation and a scaling of $\pi$ ensure that the output of the dense layer is in $[-\pi, \pi]$, which we argue in \autoref{sec:Configuring QCNs and DNNs} is a appropriate scaling for QCNs.
...
%================================================================
Unless specified otherwise, the outputs of intermediate layers of QCNs with be scaled to the interval $a_i^{(l)} \in [-\pi, \pi]$. Using the same argument about the periodicity of Pauli rotations from \autoref{sec:Initialization}, this enables(or at least makes it more likely) the outputs of one layer to realize any rotation angle in the next layer. For the last layer, the output will be scaled to $a_i^{(L)} \in [0, 1]$. This is sufficient, given that the target $y$ is scaled to within this interval. 

For DNNs, we will be using Tanh activations on all hidden layers. This is partly because it is a hughly popular choice, known to produce fast-converging neural models\cite{hands-on}. Further, Tanh is also a bounded function, returning values on the interval $[-1, 1]$, unlike the also popular activation ReLu. This results in the outputs of intermediate layers to bounded as well, like for QCNs, making them easier to compare.
