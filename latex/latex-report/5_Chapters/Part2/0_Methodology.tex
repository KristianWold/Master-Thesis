...
\textbf{circuit = ansatz(circuit, data\_reg, theta, reps)}, where \textbf{theta} is a numpy array containing the parameters. The "simple ansatz" detailed in \cref{sec:Ansätze} can be implemented as a python function:
...
%================================================================
In the QCN framework, we implement encoders, ansätze and samplers as callable python classes with much the same functionality as described in \cref{sec:QNNimpement}. We will now go through the use of the most important classes.
...
\textbf{block = ["entangle", "ry"]} will first apply CNOT gates to all neighboring qubits in sequence. $R_y$ rotations are then applied to every qubit. This particular argument recreates the simple ansatz described in \cref{sec:Ansätze}. \textbf{reps} specifies the number of times the ansatz is then repeated.
