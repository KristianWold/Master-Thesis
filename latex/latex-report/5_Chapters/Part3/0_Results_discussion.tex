...
Looking at \autoref{fig:TL_trained_C} and \autoref{fig:TL_trained_D}, we see that the corresponding DNNs fail to enter the exponential growth regime after training that matches the MSE reduction of the QCNs. The DNNs achieved exponential growth after more than an order magnitude more epochs. Even then, the trajectory length of the DNNs fell short of the QCNs. This suggests that QCNs can be trained to be more expressive than DNNs of similar number of parameters. A possible explanation for this is that the mathematical transformation applied by each node in the QCN is in a way more powerful and "interesting" compared to DNNs. As the information is represented as an entangled state in an exponentially large Hilbert space, each Pauli rotation transforms an exponential number of amplitudes. This constitutes a more complex computation than the affine transformation happening in the DNN, for the same number of parameters. However, this results only in a more interesting output for a low number of qubits, as the increased dimension of the Hilbert space tends to concentrate the output around the mean. 
